...
def epsilon_greedy_search(
    ...
    for i_episode in range(1, episode_count + 1):
        if i_episode % 10 == 0 and title != 'Random':
            print('test')
            ...
            if not os.path.exists(f'Outcome/{title}/seed{seed}/outcome.txt'):
                os.makedirs(f'Outcome/{title}/seed{seed}', exist_ok=True)
                with open(f'Outcome/{title}/seed{seed}/outcome.txt', 'w') as outcome_file:
                    pass

            with open(f'Outcome/{title}/seed{seed}/outcome.txt', 'a+') as outcome_file:
                outcome_file.write('Total Reward:%.1f\n' % reward_output)
        ...
        for t in bar(range(1, 1 + iteration_count)):
            ...
            if epsilon_exponential_decay:
                epsilon = max(epsilon_minimum, initial_epsilon + (epsilon_minimum - initial_epsilon) * steps_done / epsilon_exponential_decay)
