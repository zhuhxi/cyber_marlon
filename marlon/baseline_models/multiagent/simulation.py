...
def getCompleteGraph(cyberbattle_env: CyberBattleEnv) -> FigureWidget:
    ...
    fig.add_trace(go.Scatter(y=np.array(cyberbattle_env.env._CyberBattleEnv__episode_rewards).cumsum(),
...
def complete_network_as_plotly_traces(cyberbattle_env: CyberBattleEnv,xref: str = "x", yref: str = "y") -> Tuple[List[go.Scatter], dict]:
    env = cyberbattle_env.env._CyberBattleEnv__environment
    graph = env.network
    all_nodes = nx.DiGraph(graph)

    known_nodes_ids = [node_id for node_id, _, in cyberbattle_env.env._actuator.discovered_nodes()]
