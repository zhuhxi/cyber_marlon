...
class DefenderEnvWrapper(gym.Env, IEnvironmentObserver):
    ...
    def step(self, action) -> Tuple[Observation, float, bool, Dict[str, Any]]:
        ...
        if self.defender_constraints_broken():
            reward = self.cyber_env.env._CyberBattleEnv__LOSING_REWARD
            logging.warning("Defender Lost")
            if self.reset_on_constraint_broken:
                done = True
        if self.cyber_env.env._CyberBattleEnv__defender_goal_reached():
            reward = self.cyber_env.env._CyberBattleEnv__WINNING_REWARD
            done = True
    ...
    def defender_constraints_broken(self):
        return self.cyber_env.env._defender_actuator.network_availability < self.cyber_env.env._CyberBattleEnv__defender_constraint.maintain_sla
