...
class AttackerEnvWrapper(gym.Env, IRewardStore, IEnvironmentObserver):
    ...
    def __init__(self,
        ...
        self.bounds: EnvironmentBounds = self.cyber_env.env.bounds
        ...
        self.node_count = cyber_env.env._CyberBattleEnv__node_count
        self.__get_privilegelevel_array = cyber_env.env._CyberBattleEnv__get_privilegelevel_array
    ...
    def __create_observation_space(self, cyber_env: CyberBattleEnv) -> gym.Space:
        ...
        del observation_space['_credential_cache']
        del observation_space['_discovered_nodes']
        del observation_space['_explored_network']
    ...
    def transform_observation(self, observation) -> Observation:
        ...
        # Remove 'info' fields added by cyberbattle that do not represent algorithm inputs
        del observation['_credential_cache']
        del observation['_discovered_nodes']
        del observation['_explored_network']

        # Stable baselines does not like numpy wrapped ints
        for space in self.int32_spaces:
            if space == 'customer_data_found':
                observation[space] = int(observation[space][0])
            else:
