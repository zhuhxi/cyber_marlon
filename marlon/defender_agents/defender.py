...
class LearningDefender():
    ...
    def executeAction(self, next_action):
        self.cyber_env.env._defender_actuator.on_attacker_step_taken()
        ...
        if next_action[0] == 0:
            self.cyber_env.env._defender_actuator.reimage_node(get_node_from_action(next_action[1]))
        ...
        # If the action is a stop service.
            ...
            self.cyber_env.env._defender_actuator.stop_service(node_id, get_service_port_name_from_action(node_id, next_action[9]))

        # If the action is a start service.
        elif next_action[0] == 4:
            node_id = get_node_from_action(next_action[10])
            self.cyber_env.env._defender_actuator.start_service(node_id, get_service_port_name_from_action(node_id, next_action[11]))